class PapuaMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch("http://93.127.215.107:4000/papua");
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
            <div class="container">
              <h1>Error Loading Papua Menu</h1>
...
customElements.define("papua-menu", PapuaMenu);
